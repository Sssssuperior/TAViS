...
def evaluate_avs_synthetic(loader, model, args):
    ...
    for batch in tqdm(metric_logger.log_every(loader, 10, header)):
        img, spec, mask, video_name = batch
        ...
        gt_masks = (mask.reshape(bs*T, H, W) > 0).float()
...
def evaluate_avs_S4(loader, model, args):
    ...
    for batch in tqdm(metric_logger.log_every(loader, 10, header)):
        ...
        for idx in range(bs):
            for i in range(T):
                img_i = img[idx,i].unsqueeze(0).to(device)
                ini_img_i = ini_img[idx,i].unsqueeze(0).to(device)
                spec_i = spec[idx]
                mask_i = mask[idx,i].unsqueeze(0).to(device)

                with torch.no_grad():
                    mask_pred = model.infer(img_i, ini_img_i, [spec_i], i)
                all_pred_masks.append(mask_pred)
        all_pred_masks = torch.stack(all_pred_masks, dim=0)  # BxTxHxW
        ...
        if args.save_mask:
            assert bs == 1, "Please set batch_size to 1 when saving test masks!"
            save_dir = os.path.join(os.path.dirname(args.eval), "masks", video_name[0])
            if not os.path.exists(save_dir):
                os.makedirs(save_dir)
            for i in range(pred_masks.size(0)):
                each_mask = pred_masks[i]
