...
class TextPreprocessor(VerboseNNModule):
    ...
    def forward(self, text):
        ...
        if len(text) == 2:
            text, text_tokens = text[0], text[1]
            bs, _ = text.shape
            text = text
            text0 = torch.tensor(0, dtype=torch.long).cuda()
            text_tokens_sep = self.token_embedding(text0).repeat(bs, 71,1 )
            text_tokens = torch.cat((text_tokens, text_tokens_sep), dim=1)
        else:
