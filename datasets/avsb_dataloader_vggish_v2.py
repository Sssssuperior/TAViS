...
class V2Dataset(Dataset):
    ...
    def __init__(self, split='train', args=None):
        ...
        self.base_dir = args.config['AVSSBench']['dir_base']
        self.crop_size = args.config['AVSSBench']['crop_size']
        self.crop_img_and_mask = args.config['AVSSBench']['crop_img_and_mask']
    ...
    def __getitem__(self, index):
        ...
        img_base_path = os.path.join(
            self.base_dir, set, video_name, 'frames')
        audio_path = os.path.join(
            self.base_dir, set, video_name, 'audio.wav')
        color_mask_base_path = os.path.join(
            self.base_dir, set, video_name, 'labels_rgb')
        
        # to show the mask images and the number of frames
        ...
        for img_id in range(imgs_num):
            ...
            img, initial_img = load_image_in_PIL_to_Tensor(
                img_path, split=self.split, crop_size=self.crop_size, transform=self.img_transform, crop_img_and_mask=self.crop_img_and_mask, transform_initial = self.initial_transform)
            imgs.append(img)
        ...
        return imgs_tensor, initial_imgs_tensor, audio_path, labels_tensor, \
