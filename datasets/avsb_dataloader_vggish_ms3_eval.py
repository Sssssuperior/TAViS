...
def load_image_in_PIL_to_Tensor(path, split='train', mode='RGB', crop_size=1024, transform=None, crop_img_and_mask=True, transform_initial=None):
    ...
    #             crop_size, img_PIL, img_is_mask=False)
    #     else:
...
class MS3Dataset(Dataset):
    ...
    def __init__(self, split='train', args=None):
        ...
        df_all = pd.read_csv(args.config['AVSBenchMS3']['ANNO_CSV'], sep=',')
        ...
        self.crop_size = args.config['AVSBenchMS3']['crop_size']
        self.crop_img_and_mask = args.config['AVSBenchMS3']['crop_img_and_mask']
    ...
    def __getitem__(self, index):
        ...
        img_base_path =  os.path.join(self.args.config['AVSBenchMS3']['DIR_IMG'], video_name)
        mask_base_path = os.path.join(self.args.config['AVSBenchMS3']['DIR_MASK'])
        ...
        for img_id in range(1, self.mask_num + 1):
            img, initial_img = load_image_in_PIL_to_Tensor(os.path.join(img_base_path, "%s.mp4_%d.png"%(video_name, img_id)), transform=self.img_transform, transform_initial = self.initial_transform)
            imgs.append(img)
